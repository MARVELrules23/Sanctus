...
import { DEVOTIONS } from "@/src/prayers/devotions";
...
export default function PrayerRunnerScreen() {
  ...
  const meta: PrayerMeta = useMemo(() => {
    ...
    const dev = DEVOTIONS[kind];
    if (dev) {
      return {
        title: dev.title,
        color: dev.color,
        steps: dev.steps,
        good_work: dev.good_work_for_today,
        motto: dev.daily_motto,
        note: dev.note,
        format: dev.format,
      };
    }
  ...
  // ---- Reader mode: one calm scroll (litanies, set prayers) ---- //
  if (meta.format === "reader") {
    return (
      <View style={styles.root} testID="prayer-runner-screen">
        ...
        <SafeAreaView style={styles.safe} edges={["top", "bottom"]}>
          ...
          <ScrollView style={styles.readerScroll} contentContainerStyle={{ paddingBottom: spacing.lg }} showsVerticalScrollIndicator={false}>
            {meta.note ? (
              <View style={[styles.noteCallout, { borderLeftColor: meta.color, backgroundColor: meta.color + "14", marginHorizontal: 0 }]} testID="prayer-note">
            ...
            {meta.steps.map((s, i) => (
              <View key={i} style={styles.readerBlock} testID={`prayer-reader-block-${i}`}>
                <Text style={[styles.readerLabel, { color: meta.color }]}>{tr(s.label)}</Text>
                <Text style={styles.readerText}>{tr(s.prayer)}</Text>
              </View>
            ))}

            <View style={[styles.readerGoodWork, { borderColor: meta.color + "55", backgroundColor: meta.color + "10" }]} testID="prayer-good-work">
              <Text style={[styles.eyebrow, { color: meta.color }]}>{ui.goodWorkUpper}</Text>
              <Text style={styles.readerGoodWorkText}>{tr(meta.good_work)}</Text>
              {meta.motto ? (
                <View style={[styles.mottoBox, { borderTopColor: meta.color + "33" }]}>
...
const styles = StyleSheet.create({
  ...
  readerScroll: { flex: 1, marginTop: spacing.sm },
  readerBlock: { backgroundColor: "#FAF9F6", borderRadius: radius.lg, padding: spacing.md, marginBottom: spacing.sm },
  readerLabel: { fontFamily: fonts.headingSemi, fontSize: 15, marginBottom: 6, letterSpacing: 0.2 },
  readerText: { fontFamily: fonts.bodyRegular, fontSize: 16, lineHeight: 26, color: colors.textPrimary },
  readerGoodWork: { borderWidth: 1, borderRadius: radius.lg, padding: spacing.md, marginTop: spacing.xs },
  readerGoodWorkText: { fontFamily: fonts.bodyRegular, fontSize: 15, lineHeight: 24, color: colors.textPrimary, marginTop: 4 },
