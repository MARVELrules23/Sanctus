...
## Session update 30 (July 2026) — Backlog batch (companion practices, daily acts, virtue tz, pilgrimage, RN-web warnings)
- **Add to my practices** (companion page): church_traditions, daily_traditions, vocation_traditions rows now have "Add to my practices" button (testID companion-add-practice-{ct|dt|vt}-{i}). Backend `POST /api/my/devotions/add-practice` {saint_name, saint_slug, practice} appends to (or creates) the user's devotion for that saint. Note: only saints with populated traditions show the button (e.g. blessed-virgin-mary, st-joseph, francis-assisi); augustine/thomas-aquinas have ct/dt=0.
- **Daily companions' today's act on Home**: `GET /api/companions/daily-acts` returns permanent Guardian Angel + selected daily companions with today's act. Rendered in VocationHomeCard "TODAY'S ACTS WITH YOUR COMPANIONS" (testID vocation-daily-acts, rows daily-act-{slug}), tap → /companion/{slug}. NOTE: only shows when user has a vocation guide (card returns null otherwise).
- **Virtue "today" count timezone fix**: VirtusHomeCard + virtus/plan/[id].tsx now compute count from `plan.checkins[todayISO()]` (local date) instead of backend server-UTC `completed`/selected-day count.
- **Pilgrimage day-before reminder + mark completed**: schedule.py adds `completed`/`completed_at` + `POST /api/schedule/{id}/complete`. notifications.ts schedules a gentle day-before (09:00) reminder for kind=pilgrimage. map.tsx planPilgrimage now sets time 09:00 + notify:true and schedules OS notifications + saves notif ids. schedule/index.tsx shows "Mark as completed"/"Pilgrimage completed" chip (testID schedule-complete-{id}) on pilgrimage items with strike-through title when done.
- **RN-web deprecations**: theme.ts shadow.card now Platform.select (boxShadow on web); coloring Svg pointerEvents moved into style.
- Backend verified via curl: daily-acts, add-practice, schedule complete all 200.

## Session update 31 (July 2026) — 15 new prayers + companion deep-links
- Added 13 devotions to src/prayers/devotions.ts (reader format) + registry + DEVOTION_CATEGORIES: to_you_blessed_joseph, angel_of_god, litany_virgin_martyrs, act_of_reparation, apostles_creed, chair_of_st_peter, stay_with_me_lord, the_little_way, holy_face, consecration_of_purity, benedictus, little_office_bvm (explains Brown Scapular), tantum_ergo.
